...
/** 设备地址（4字节），默认广播地址0xFFFFFFFF，可通过模块配置修改 */
uint8_t g_deviceAddress[4] = {0xFF, 0xFF, 0xFF, 0xFF};
/** 已注册指纹ID数组，最大支持100枚（0-99），未使用位置为0xFF */
uint8_t g_fingerIDArray[100] = {0xFF};
/** 当前有效指纹数量 */
uint8_t g_fingerNumber = 0;
...
esp_err_t verify_received_data(const uint8_t *recvData, uint16_t dataLen)
{
    ...
    {
        printf("校验失败：数据为空或长度不足（最小需%d字节，当前%d字节）\n",
               MIN_RESPONSE_LEN, dataLen);
        return ESP_FAIL;
    }
    ...
    {
        printf("校验失败：帧头不匹配（期望%02X%02X，实际%02X%02X）\n",
    ...
    for (int i = 2; i < 6; i++)
    {
        ...
        {
            printf("校验失败：设备地址不匹配（期望%02X%02X%02X%02X，实际%02X%02X%02X%02X）\n",
    ...
    {
        printf("校验失败：包标识错误（期望应答包%02X，实际%02X）\n",
    ...
    uint16_t expectedTotalLen = 9 + expectedDataLen + CHECKSUM_LEN; // 总长度=帧头(2)+地址(4)+包标识(1)+长度(2)+数据区+校验和(2)
    if (expectedTotalLen != dataLen)
    {
        printf("校验失败：长度不匹配（期望总长度%d，实际%d）\n",
               expectedTotalLen, dataLen);
        return ESP_FAIL;
    }
    ...
    // 校验和计算范围：包标识(6)到数据区末尾(dataLen-3)
    for (uint16_t i = CHECKSUM_START_INDEX; i <= dataLen - 3; i++)
    {
    ...
    {
        printf("校验失败：校验和不匹配（期望0x%04X，实际0x%04X）\n",
 ...
 */
uint16_t calculate_checksum(const uint8_t *frameData, uint16_t frameLen)
{
 ...
 * @return esp_err_t 操作结果：ESP_OK=帧组装成功，ESP_FAIL=参数无效或组装失败
 */
esp_err_t control_colorful_led(uint8_t startColor, uint8_t timeBit, uint8_t cycleTimes)
{
    // 参数合法性检查
 ...
 * @return esp_err_t 操作结果：ESP_OK=帧组装成功，ESP_FAIL=参数无效或组装失败
 */
esp_err_t delete_char(uint16_t ID, uint16_t count) // 修正函数名拼写错误（delet→delete）
{
    // 参数合法性检查
    if (ID >= 100)
    {
...
esp_err_t fingerprint_parse_frame(const uint8_t *recvData, uint16_t dataLen)
{
    ...
    for (uint8_t i = 10; i <= 22; i++) // i=10到22共13字节
    {
        ...
        for (uint8_t j = 0; j < 8; j++)
        {
            if (byteData & mask[j])
            {
                ...
                {
                    g_fingerIDArray[tempCount] = fingerID;
                    tempCount++;
                    if (tempCount >= 100)
                        break; // 达到最大容量则停止
                }
    ...
    // 更新有效指纹数量
    g_fingerNumber = tempCount;

    // 打印解析结果
    if (g_fingerNumber > 0)
    {
        printf("检测到%d个已注册指纹ID: ", g_fingerNumber);
