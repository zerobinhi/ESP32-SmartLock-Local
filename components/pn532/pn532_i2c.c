...
// 准备添加卡
bool READY_ADD_CARD = 0;
// 卡号，最多添加19个卡
char CARD_ID[20][11] = {0};
...
esp_err_t pn532_initialization()
{
    ...
    pn532_send_command_and_receive(CMD_READ1, sizeof(CMD_READ1), ack, sizeof(ack));
            ...
            if (i2c_master_receive(pn532_handle, res, sizeof(res), 100) == ESP_OK && res[0] == 0x01)
            {
                ...
                if (READY_ADD_CARD == 1) // 添加卡操作
                {
                    if (find_card_id(PN532_UID, CARD_NUMBER) == 0) // 没有此卡
                    {
                        ...
                        int storage_index = CARD_NUMBER + 1; // 实际存储位置
                        ...
                        ESP_LOGI(TAG, "添加卡片: %s 到位置: %d", CARD_ID[storage_index], storage_index);
                        CARD_NUMBER++; // 增加卡片数量
                        // write_nvs_i32("CARD_NUMBER", CARD_NUMBER);

                        char temp[20];
                        snprintf(temp, sizeof(temp), "CUID_ID%d", storage_index);
                ...
                {
                    ESP_LOGI(TAG, "CARD_NUMBER: %d", CARD_NUMBER);
                    ESP_LOGI(TAG, "PN532_UID: %s", PN532_UID);
